...
```
lib/staff_forecast_web/               ← Controllers, LiveViews, components. Thin.
                                        Parse, authorize, dispatch. No business logic.
lib/staff_forecast/                   ← Domain. One directory per context.
...
- `staff_forecast_web/` may only call the public API of contexts in `staff_forecast/`.
...
Every Command and Query should take a `%StaffForecast.Ctx{}` struct as its first argument, carrying:

- `correlation_id` (UUID, generated at request entry)
- `user_id` / `current_scope` slice

The **first action** of every Command/Query is `StaffForecast.Ctx.stamp_metadata(ctx)`, which mirrors the context into `Logger.metadata/1`. Builders never touch `Logger.metadata` by hand — the helper is the single call site.
...
1. **Expected failures return tagged tuples.** Commands and Queries return `{:ok, result} | {:error, reason}`. `reason` is a **domain error struct** under `lib/staff_forecast/<context>/errors/` — defined via `defexception` so it's *both* pattern-matchable as `{:error, struct}` **and** raisable when exceptional.
   ```elixir
   defmodule StaffForecast.Forecasts.Errors.InvalidPeriod do
...
3. **Adapters never leak vendor exceptions.** Every adapter catches vendor errors, wraps in a domain error struct, and returns `{:error, %<Context>.Errors.<Name>{cause: original}}`. A vendor exception leaking into `/lib/staff_forecast/<context>/commands/` → fail.
...
- Tests live in `/test/` and **mirror `/lib/`** structure: `test/staff_forecast/forecasts/commands/create_forecast_test.exs` ↔ `lib/staff_forecast/forecasts/commands/create_forecast.ex`.
...
- **Modules**: `PascalCase`, role carried in the namespace:
  - `StaffForecast.Forecasts.Commands.CreateForecast`
  - `StaffForecast.Forecasts.Queries.ListForecasts`
  - `StaffForecast.Forecasts.Ports.ScheduleProvider`
  - `StaffForecast.Forecasts.Adapters.HRSystem`
  - `StaffForecast.Forecasts.Contracts.CreateForecastInput`
  - `StaffForecast.Forecasts.Errors.InvalidPeriod`
...
3. **Schema** lives in `lib/staff_forecast/<context>/schema/`, one file per table. `Ecto.Schema` fields always use `:string` for text columns (per `AGENTS.md`).
...
1. **Single module — `StaffForecast.Config`** under `lib/staff_forecast/config.ex`. The *only* module in the app permitted to call `Application.get_env/2`. (Establish this module in the first sprint that needs config access.)
2. **`config/runtime.exs`** is the *only* place `System.get_env/1` / `System.fetch_env!/1` may appear.
3. **Validation**: `StaffForecast.Config` should validate required config at boot. `NimbleOptions` is available if needed.
...
Every external input — inbound and outbound — is validated via an `Ecto.Changeset` against an embedded schema under `lib/staff_forecast/<context>/contracts/`.

**Rules:**
- `/lib/staff_forecast/<context>/` functions **never** accept raw `params` maps. They take a validated struct.
