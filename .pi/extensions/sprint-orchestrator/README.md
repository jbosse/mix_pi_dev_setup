...
| `sprint_state_unhalt` | Clear the halted flag after a human-approved fix. For strike-4 halts, also resets the in-flight task's gate/strikes back to builder so the gate chain can re-run cleanly. |
...
| `/sprint:unhalt` | Clear the halted flag after a human-approved fix. For strike-4 halts, also resets the in-flight task gate/strikes to builder. Pass a brief description of what was fixed. |
