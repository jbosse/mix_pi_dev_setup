...
```
lib/pi_dev_setup_web/               ← Controllers, LiveViews, components. Thin.
                                        Parse, authorize, dispatch. No business logic.
lib/pi_dev_setup/                   ← Domain. One directory per context.
lib/pi_dev_setup/<context>/
...
- `pi_dev_setup_web/` may only call the public API of contexts in `pi_dev_setup/`.
...
Every Command and Query should take a `%PiDevSetup.Ctx{}` struct as its first argument, carrying:

- `correlation_id` (UUID, generated at request entry)
- `user_id` / `current_scope` slice

The **first action** of every Command/Query is `PiDevSetup.Ctx.stamp_metadata(ctx)`, which mirrors the context into `Logger.metadata/1`. Builders never touch `Logger.metadata` by hand — the helper is the single call site.
...
1. **Expected failures return tagged tuples.** Commands and Queries return `{:ok, result} | {:error, reason}`. `reason` is a **domain error struct** under `lib/pi_dev_setup/<context>/errors/` — defined via `defexception` so it's *both* pattern-matchable as `{:error, struct}` **and** raisable when exceptional.
   ```elixir
   defmodule PiDevSetup.Forecasts.Errors.InvalidPeriod do
...
3. **Adapters never leak vendor exceptions.** Every adapter catches vendor errors, wraps in a domain error struct, and returns `{:error, %<Context>.Errors.<Name>{cause: original}}`. A vendor exception leaking into `/lib/pi_dev_setup/<context>/commands/` → fail.
...
- Tests live in `/test/` and **mirror `/lib/`** structure: `test/pi_dev_setup/forecasts/commands/create_forecast_test.exs` ↔ `lib/pi_dev_setup/forecasts/commands/create_forecast.ex`.
...
- **Modules**: `PascalCase`, role carried in the namespace:
  - `PiDevSetup.Forecasts.Commands.CreateForecast`
  - `PiDevSetup.Forecasts.Queries.ListForecasts`
  - `PiDevSetup.Forecasts.Ports.ScheduleProvider`
  - `PiDevSetup.Forecasts.Adapters.HRSystem`
  - `PiDevSetup.Forecasts.Contracts.CreateForecastInput`
  - `PiDevSetup.Forecasts.Errors.InvalidPeriod`
...
3. **Schema** lives in `lib/pi_dev_setup/<context>/schema/`, one file per table. `Ecto.Schema` fields always use `:string` for text columns (per `AGENTS.md`).
...
1. **Single module — `PiDevSetup.Config`** under `lib/pi_dev_setup/config.ex`. The *only* module in the app permitted to call `Application.get_env/2`. (Establish this module in the first sprint that needs config access.)
2. **`config/runtime.exs`** is the *only* place `System.get_env/1` / `System.fetch_env!/1` may appear.
3. **Validation**: `PiDevSetup.Config` should validate required config at boot. `NimbleOptions` is available if needed.
...
Every external input — inbound and outbound — is validated via an `Ecto.Changeset` against an embedded schema under `lib/pi_dev_setup/<context>/contracts/`.

**Rules:**
- `/lib/pi_dev_setup/<context>/` functions **never** accept raw `params` maps. They take a validated struct.
